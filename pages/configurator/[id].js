...
const Configurator = () => {
    ...
    const data =
        filteredZaunseriesPfoste?.length > 0
            ? filteredZaunseriesPfoste.map(
                  (z) =>
                      z?.reference?.fields.find((f) => f.key === 'pfosten')
                          .references.edges[0]
              )
            : null
    ...
    const configuratorPfosten = configuratorItems.map((pfoste) => {
        ...
        return {
            ...
            height: pfoste.node.options.find((f) => f.name === 'Höhe')
                .values[0],
            options: allOptions,
            variantTitle: pfoste.pfosten.node.title,
            variantPrice:
                pfoste.pfosten.node.variants.edges[0].node.priceV2.amount,
            variantQuantity: 1,
        }
    })
        ...
        <div className="container">
            {loading ? (
                ...
                <>
                    ...
                    <ConfiguratorOptions
                        filteredZaunserie={filteredZaunseriesPfoste}
                    />
                    ...
                    <div className={styles.ConfiguratorFooter}>
                        <div
                            className={`${styles.ConfiguratorFooter} ${styles.Buttons}`}
                        >
