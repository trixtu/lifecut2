...
export async function createCheckoutOneProduct(id, quantity) {
  const query = `
  mutation {
...
export async function createCheckout(id, quantity) {
  const lineItemsObject = quantity.map((item) => {
    return `{
        variantId: "${item.id}",
...
export async function updateCheckoutMultiple(id, lineItems) {
  ...
  const query = `
  mutation {
    checkoutLineItemsReplace(lineItems: [${lineItemsObject}]) {
      checkout {
        id
        webUrl
        lineItems(first: 100) {
